...
import { MiddlewareHandler, ZodSchema, z } from "../deps.ts";
...
export const validatorMiddleware = (): MiddlewareHandler => {
  return async function validatorMiddleware(c, next) {
    ...
    let value
    let target
    let schema = getZodValidatorSchema(taskType, gatewayParams.model as Provider, Target.JSON);
    if (schema) {
      value = await c.req.json()
      target = [Target.JSON]
    } else {
      schema = getZodValidatorSchema(taskType, gatewayParams.model as Provider, Target.FORM)
      if (schema) {
