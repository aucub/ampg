...
import { IChatService, IEmbeddingService, IImageEditService, ITranscriptionService } from "../types/i_service.ts";

const DEFAULT_CLOUDFLARE_BASE_URL = 'https://api.cloudflare.com/client/v4/accounts/'

const DEFAULT_CLOUDFLAREWORKERSAI_CHAT_MODEL = "@cf/meta/llama-2-7b-chat-int8";

const DEFAULT_CLOUDFLAREWORKERSAI_EMBEDDING_MODEL = "@cf/baai/bge-large-en-v1.5";
...
export class CloudflareWorkersAIEmbeddingService implements IEmbeddingService {
  ...
  async executeModel(c: Context, params: EmbeddingParams): Promise<number[] | number[][]> {
    const modelName = cloudflareWorkersAITextEmbeddingsModel.includes(params.modelName ?? "")
      ? params.modelName
...
export class CloudflareWorkersAITranscriptionService implements ITranscriptionService {
  ...
  async executeModel(c: Context, params: TranscriptionParams): Promise<any> {
    const modelName = cloudflareWorkersAISpeechRecognitionModel.includes(params.modelName ?? "")
      ? params.modelName
